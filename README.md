...
> **Rules → Distilled tiny-BERT → LLM 终审**：一条流水线同时拿到意图、置信度、结构化槽位与风险标记。
> 90% 的请求在 **30ms** 内于 CPU 上解决，只有真正困难的 10% 才升级大模型——成本、延迟、精度三者兼得。
...
- 🧠 **师生蒸馏 + 多任务联合建模**：bert-base-chinese 教师的"模糊认知"通过软标签（KL 散度，T=2.0）蒸馏进 3 层学生模型；学生采用**意图分类头 + BIO 序列标注头双头架构**，一次前向同时输出意图与 subject/grade 实体槽位——槽位标注零人工成本（词典远程监督自动生成 BIO 标签）
- ⚡ **置信短路路由（本架构的灵魂）**：意图 ≥0.85 且槽位 ≥0.80 且非复杂长句 → 小模型结果直接放行；低置信 / 槽位不稳 / 复杂 query → LLM 终审兜底（错就修正，LLM 拥有终审权）。实测约九成请求被 30ms 短路解决
- 🛡️ **零延迟风险拦截**：作弊话术、心理高危词在第一层 0ms 命中，直接携带合规话术结束链路——大模型永远不碰不该碰的请求
- 📦 **稳定契约的网关设计**：无状态 HTTP 服务 `POST /classify`，输出 pydantic 强类型 `IntentResult`（含 `decision_trace` 全链路决策路径，可解释可排查）；二期升级模型时 schema 不变，下游零改动
- 🎬 **完整可玩的演示应用**：FastAPI + LangGraph 5 节点编排（多轮槽位记忆 cached_slots、反问补槽、脚手架式答疑守卫、SSE 流式打字机）+ 零依赖网页多会话聊天——`python run_demo.py` 一键起飞
...
如果这个项目对你有帮助，**欢迎 Star ⭐ 支持**——这是持续迭代最快的动力。
